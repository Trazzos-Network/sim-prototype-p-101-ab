...
export function generateSensorData(): SensorData[] {
  ...
  for (let frame = 0; frame < TOTAL_FRAMES; frame++) {
    ...
    const vibration = generateVibration(frame, rng);

    // Generate pressure (inverse correlation with vibration)
    const pressure = generatePressure(vibration, frame, rng);

    // Generate motor current (slight positive correlation)
    const motorCurrent = generateMotorCurrent(vibration, frame, rng);
...
function generateVibration(frame: number, rng: SeededRandom): number {
  ...
  if (frame < MAINTENANCE_FRAME) {
    ...
    const noise = (rng.random() - 0.5) * 0.15; // ±0.075 noise
...
function generatePressure(
  ...
  if (frame < MAINTENANCE_FRAME) {
    ...
    const noise = (rng.random() - 0.5) * 0.03;
  ...
  const noise = (rng.random() - 0.5) * 0.025;
  return Math.max(0.85, Math.min(1.0, basePressure + noise));
}

function generateMotorCurrent(
  vibration: number,
  frame: number,
  rng: SeededRandom
): number {
  ...
  const noise = (rng.random() - 0.5) * 0.8;
